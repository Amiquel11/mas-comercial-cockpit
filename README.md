...
- `src/ingestion/health/function.json` must exist.
- `src/ingestion/pipedrive_ingestion_timer/function.json` must exist.
...
- Confirm `src/ingestion/host.json` includes an extension bundle for non-HTTP triggers (Timer uses extension bundle) and function folders include `function.json` files.
- Restart the Function App after deployment and wait 1-3 minutes for indexing.
- After RBAC deploy, run `sync-functions` and list functions to confirm indexing (the workflow now does this automatically).


To maximize compatibility with Azure indexing on Linux Flex Consumption, functions are declared with the classic v1 folder-based model (`<function_name>/function.json` + `__init__.py`).
